...
**⚠️ Seit Version 1.4.6:** Alle Zähler benötigen einen Namen (Standard: "main")

```
utility-monitor.0.
├── gas/
│   └── main/                    ← Hauptzähler (Name konfigurierbar)
│       ├── consumption/ (daily, monthly, yearly)
│       ├── costs/ (daily, monthly, yearly, totalYearly, basicCharge)
│       ├── info/ (meterReading, currentPrice, lastSync)
│       ├── billing/ (endReading, closePeriod, periodEnd)
│       └── statistics/ (averageDaily, averageMonthly)
├── water/
│   └── main/                    ← Gleiche Struktur
└── strom/
    └── main/                    ← Gleiche Struktur
```

**Bei mehreren Zählern:**
```
utility-monitor.0.
├── gas/
│   ├── main/                    ← Hauptzähler
│   ├── werkstatt/               ← Zusätzlicher Zähler
│   └── totals/                  ← Summen aller Zähler
```
