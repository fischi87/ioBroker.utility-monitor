...
class MultiMeterManager {
    ...
    async initializeMeter(type, meterName, config, displayName) {
        ...
        // IMPORTANT: Only restore if the value is > 0, otherwise treat as first-time setup
        // This prevents the adapter from treating the entire sensor value as "consumption"
        // when a meter is first configured
        const lastReading = await this.adapter.getStateAsync(`${basePath}.info.meterReading`);
        if (lastReading && typeof lastReading.val === 'number' && lastReading.val > 0) {
            ...
            // First-time setup or meterReading is 0 - don't set lastSensorValues
            // This will trigger _handleFirstSensorValue in handleSensorUpdate
            this.adapter.log.debug(`${type}.${meterName}: First-time setup, will use sensor value as baseline`);
        }
        ...
        // Update billing countdown (shared implementation, refreshed every minute
        // by billingManager.checkPeriodResets())
        if (config.contractStart) {
            const countdown = await this.billingManager.updateCountdownForMeter(basePath, config.contractStart);

            if (countdown) {
                this.adapter.log.debug(
                    `[${basePath}] Billing countdown: contractStart=${config.contractStart}, daysRemaining=${countdown.daysRemaining}, periodEnd=${countdown.periodEnd.toLocaleDateString('de-DE')}`,
                );
            }
        }

        // Set monthly installment (Abschlag) as a plain number so it can be
        // logged, charted and used in scripts (#11). The unit is on the object.
        await this.adapter.setStateAsync(
            `${basePath}.info.monthlyInstallment`,
            calculator.roundToDecimals(config.abschlag, 2),
            true,
        );
    ...
    async _handleFirstSensorValue(type, meterName, sensorDP, processed, basePath, config, now) {
        ...
        if (recoveredValue > 0 && Math.abs(consumption - recoveredValue) < 100) {
            ...
            // This is either first-time setup (recoveredValue = 0) or a major sensor change
            const isFirstTimeSetup = recoveredValue === 0 || recoveredValue === null;
            ...
            // Set the current sensor value as the baseline (no delta calculation)
            this.lastSensorValues[sensorDP] = consumption;
            await this.adapter.setStateAsync(`${basePath}.info.meterReading`, consumption, true);
            if (type === 'gas') {
                await this.adapter.setStateAsync(`${basePath}.info.meterReadingVolume`, consumptionM3 || 0, true);
            ...
            // For first-time setup: Reset period counters to 0
            // This prevents the sensor's absolute value from being treated as consumption
            if (isFirstTimeSetup) {
                this.adapter.log.info(`[${basePath}] First-time setup detected. Resetting period counters to 0.`);
                ...
                // Also reset costs for the periods
                ...
                // On baseline reset (not first-time), validate period consumption values
                await this._validatePeriodConsumption(type, basePath, now);
            }
    ...
    async _handleMeterReset(type, meterName, lastValue, consumption, config) {
        this.adapter.log.warn(
            `${type}.${meterName}: Meter reading decreased (${lastValue} -> ${consumption}). Assuming meter change or reset.`,
        );
        await this.updateCosts(type, meterName, config);
        await this.updateTotalCosts(type);
    }
