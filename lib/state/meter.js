...
async function migrateInstallmentToNumber(adapter, id) {
    try {
        ...
        adapter.log.info(`${id}: von Text auf Zahl umgestellt`);
    } catch (error) {
        adapter.log.warn(`${id} konnte nicht auf Zahl umgestellt werden: ${error.message}`);
    }
}
...
async function createMeterStructure(adapter, type, meterName, _config) {
    ...
    if (isGas) {
        const volumeStates = [
            ...
            { id: 'weeklyVolume', name: 'Wöchentlicher Verbrauch (m³)' },
    ...
    await adapter.setObjectNotExistsAsync(`${basePath}.info.monthlyInstallment`, {
        type: 'state',
        common: {
            name: 'Monatliche Abschlagszahlung',
            type: 'number',
            role: STATE_ROLES.cost,
            read: true,
            write: false,
