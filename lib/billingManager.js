...
class BillingManager {
    ...
    async closeBillingPeriodForMeter(type, meter) {
        ...
        this.adapter.log.info(`🔔 Closing billing period for ${basePath} (${label})...`);
        ...
        if (!endReading || endReading <= 0) {
            this.adapter.log.error(`❌ No valid final meter reading for ${basePath}. Please enter a value first!`);
        ...
        if (!contractStartDate) {
            this.adapter.log.error(`❌ No contract start configured for ${basePath}.`);
            await this.adapter.setStateAsync(`${basePath}.billing.closePeriod`, false, true);
            return;
        }
        ...
        if (!startDate) {
            this.adapter.log.error(`❌ Invalid date format for contract start: ${contractStartDate}`);
            await this.adapter.setStateAsync(`${basePath}.billing.closePeriod`, false, true);
            return;
        }
        ...
        await this.adapter.setObjectNotExistsAsync(`${basePath}.history.${year}.yearly`, {
            type: 'state',
            common: {
                name: tr(`Yearly consumption ${year}`, `Jahresverbrauch ${year}`),
                type: 'number',
                role: 'value',
                read: true,
                write: false,
        ...
        await this.adapter.setObjectNotExistsAsync(`${basePath}.history.${year}.totalYearly`, {
            type: 'state',
            common: {
                name: tr(`Total costs ${year}`, `Gesamtkosten ${year}`),
                type: 'number',
                role: 'value',
                read: true,
                write: false,
        ...
        this.adapter.log.info(`✅ Billing period ${year} for ${basePath} closed successfully!`);
        this.adapter.log.info(
            `💡 Tip: Check your adapter configuration! Did your tariff, installment or basic charge change?`,
        );
    }

    /**
     * Updates the billing countdown states for a single meter.
     * This is the single source of truth for daysRemaining/periodEnd -
     * both the initial setup and the periodic refresh go through here.
     *
     * States are only written when the value actually changed, so the
     * minutely refresh does not cause needless database churn.
     *
     * @param {string} basePath - Meter base path (e.g. 'gas.main')
     * @param {string} contractStart - Contract start as German date string
     * @returns {Promise<object|null>} Countdown info or null if the date is invalid
     */
    async updateCountdownForMeter(basePath, contractStart) {
        const countdown = calculator.calculateBillingCountdown(contractStart);
        if (!countdown) {
            return null;
        }

        const periodEndText = countdown.periodEnd.toLocaleDateString('de-DE');

        const daysState = await this.adapter.getStateAsync(`${basePath}.billing.daysRemaining`);
        if (daysState?.val !== countdown.daysRemaining) {
            await this.adapter.setStateAsync(`${basePath}.billing.daysRemaining`, countdown.daysRemaining, true);
        }

        const endState = await this.adapter.getStateAsync(`${basePath}.billing.periodEnd`);
        if (endState?.val !== periodEndText) {
            await this.adapter.setStateAsync(`${basePath}.billing.periodEnd`, periodEndText, true);
        }
    ...
    async updateBillingCountdown(type) {
        ...
        for (const meter of meters) {
            ...
            await this.updateCountdownForMeter(`${type}.${meter.name}`, contractStart);
        }
    }
    ...
    async checkPeriodResets() {
        ...
        for (const type of types) {
            ...
            // Keep the billing countdown current while the adapter runs.
            // Without this the value stays frozen at whatever it was on adapter
            // start and never rolls over into the next contract period (#9).
            ...
            if (lastDayStart?.val) {
                ...
                if (isResetTime && !alreadyResetToday) {
                    this.adapter.log.info(`Daily reset for ${type} at 23:59`);
                    ...
                    this.adapter.log.info(`Daily reset for ${type} (catch-up - adapter was offline)`);
            ...
            if (lastWeekStart?.val) {
                ...
                if (isSunday && isResetTime && needsWeeklyReset) {
                    this.adapter.log.info(`Weekly reset for ${type} at 23:59`);
                    ...
                    this.adapter.log.info(`Weekly reset for ${type} (catch-up)`);
            ...
            if (meters.length > 0) {
                ...
                if (lastMonthStartState?.val) {
                    ...
                    // Check if the last reset was a valid 23:59 reset
                    // to prevent adapter restarts from blocking the real monthly reset
                    const wasValidMonthlyReset = lastMonthDate.getHours() === 23;

                    // Calculate what month the last reset was FOR (not when it happened)
                    // A reset at 23:59 on Jan 31 is FOR February (the month AFTER the reset date)
                    // We use modulo arithmetic to avoid JavaScript Date overflow issues
                    // (e.g., Jan 31 + 1 month = Mar 3, not Feb 28)
                    let lastResetForMonth;
                    if (wasValidMonthlyReset) {
                        // Valid 23:59 reset - this was for the NEXT month
                        // Use modulo to safely get next month (0-11)
                        lastResetForMonth = (lastMonthDate.getMonth() + 1) % 12;
                    } else {
                        // Invalid reset time - use the month of the timestamp
                        lastResetForMonth = lastMonthDate.getMonth();
                    }

                    // Check if reset is needed for CURRENT month
                    // Reset is needed if the last reset was NOT for the current month
                    const currentMonth = nowDate.getMonth();
                    const resetAlreadyDoneForCurrentMonth = lastResetForMonth === currentMonth;

                    // For last day of month: check if reset needed for NEXT month
                    const nextMonthAfterNow = (nowDate.getMonth() + 1) % 12;
                    const needsResetForNextMonth = isLastDayOfMonth && lastResetForMonth !== nextMonthAfterNow;

                    if (isLastDayOfMonth && isResetTime && needsResetForNextMonth) {
                        this.adapter.log.info(`Monthly reset for ${type} at 23:59`);
                        await this.resetMonthlyCounters(type, getNormalizedResetTime(nowDate));
                    } else if (!resetAlreadyDoneForCurrentMonth && nowDate.getDate() > 1) {
                        // Catch-up: We're past the 1st of a new month and haven't reset yet for THIS month
                        ...
                        this.adapter.log.info(`Monthly reset for ${type} (catch-up)`);
                    ...
                    if (contractStartDate) {
                        ...
                        if (contractStart) {
                            ...
                            if (isTodayDayBefore && isResetTime && needsReset) {
                                this.adapter.log.info(
                                    `Yearly reset for ${meterBasePath} at 23:59 (contract anniversary tomorrow: ${contractStartDate})`,
                                );
                                ...
                                this.adapter.log.info(
                                    `Yearly reset for ${meterBasePath} (catch-up - anniversary: ${contractStartDate})`,
                                );
                        ...
                        if (isDecember31 && isResetTime && !needsReset) {
                            ...
                            this.adapter.log.info(`Yearly reset for ${meterBasePath} at 23:59 (calendar year)`);
                            await this.resetYearlyCountersForMeter(type, meter, getNormalizedResetTime(nowDate));
                            ...
                            this.adapter.log.info(`Yearly reset for ${meterBasePath} (catch-up - calendar year)`);
                            await this.resetYearlyCountersForMeter(type, meter, catchUpTime);
