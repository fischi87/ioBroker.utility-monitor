...
class ImportManager {
    ...
    async handleImportCSV(obj) {
        ...
        // The Admin import panel uses a native jsonConfig "sendTo" button with
        // useNative:true, so obj.message carries the saved native config
        // (importType / importMeter / importCsvContent). The legacy custom
        // component sent type / meterName / content instead, which is still
        // accepted for backward compatibility.
        const msg = obj.message;
        const type = msg.type || msg.importType;
        const meterName = (msg.meterName || msg.importMeter || '').trim();
        const content = msg.content || msg.importCsvContent;
