...
class ImportManager {
    ...
    async handleImportCSV(obj) {
        ...
        // The Admin import panel sends type + meterName via the jsonConfig
        // "sendTo" button (jsonData). The (potentially multi-line) CSV content is
        // NOT sent in the message - it is read from the saved native config
        // (importCsvContent), which avoids fragile string escaping in jsonData.
        // The legacy custom component sent type / meterName / content directly,
        // which is still accepted for backward compatibility.
        const msg = obj.message;
        const cfg = this.adapter.config || {};
        const type = msg.type || msg.importType || cfg.importType;
        const meterName = (msg.meterName || msg.importMeter || cfg.importMeter || '').trim();
        const content = msg.content || msg.importCsvContent || cfg.importCsvContent;
        const format = msg.format || 'generic';

        this.adapter.log.info(`[Import] Starting CSV import for ${type}.${meterName} (Format: ${format})`);

        try {
            if (!type || !meterName) {
                throw new Error(
                    this.L(
                        'Please select a utility type and enter a meter name.',
                        'Bitte Verbrauchsart wählen und einen Zählernamen eintragen.',
                    ),
                );
            }
            if (!content || !content.trim()) {
                throw new Error(
                    this.L(
                        'No CSV content. Paste the data and press SAVE before importing.',
                        'Kein CSV-Inhalt. Bitte Daten einfügen und vor dem Import SPEICHERN.',
                    ),
                );
            }
            ...
            result.result = this.L(
                `✅ Import successful: ${result.count} records (${result.first} – ${result.last}).`,
                `✅ Import erfolgreich: ${result.count} Datensätze (${result.first} – ${result.last}).`,
            );
    ...
    /**
     * Picks the English or German variant based on the ioBroker system language.
     ...
     */
    L(en, de) {
        return (this.adapter.language || '').toString().toLowerCase() === 'de' ? de : en;
    }
    ...
    async processImport(type, meterName, content, _format) {
        ...
        if (lines.length < 2) {
            throw new Error('The file is empty or contains too little data.');
        }
        ...
        if (dataPoints.length === 0) {
            ...
            throw new Error(
                'No valid data points found. Please make sure the file contains columns for date and value.',
            );
        }
    ...
    async ensureMeterObjects(type, meterName) {
        ...
        const typeName = {
            ...
            electricity: { en: 'Electricity', de: 'Strom' },
            pv: { en: 'PV', de: 'PV' },
        };
        ...
        await this.adapter.setObjectNotExistsAsync(type, {
            type: 'channel',
            common: { name: typeName[type] || type },
            native: {},
        });
        ...
        await this.adapter.setObjectNotExistsAsync(basePath, {
            type: 'channel',
            common: { name: { en: `Meter: ${meterName}`, de: `Zähler: ${meterName}` } },
            native: { meterName },
        });
        ...
        await this.adapter.setObjectNotExistsAsync(`${basePath}.lastImport`, {
            type: 'state',
            common: {
                name: { en: 'Last import', de: 'Letzter Import' },
                type: 'number',
                role: 'value.time',
                read: true,
                write: false,
