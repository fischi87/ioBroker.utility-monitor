...
class MessagingHandler {
    ...
    /**
     * Returns the active language for outgoing user messages. English is the
     * default; German is used only when the ioBroker system language is German.
     *
     * @returns {'en'|'de'} Language code
     */
    lang() {
        const l = (this.adapter.language || '').toString().toLowerCase();
        return l === 'de' ? 'de' : 'en';
    }

    /**
     * Picks the English or German variant based on the active language.
     *
    ...
    async handleMessage(obj) {
        ...
        if (obj.command === 'getMeters') {
            ...
            try {
                ...
                const result = [{ value: '', label: this.L('none', 'kein') }];
                ...
                this.adapter.sendTo(
                    obj.from,
                    obj.command,
                    [{ value: '', label: this.L('Error', 'Fehler') }],
                    obj.callback,
                );
            }
            ...
            try {
                ...
                if (!instance || instance === 'none' || instance === 'kein') {
                    this.adapter.sendTo(
                        ...
                        {
                            error: this.L(
                                'No instance selected. Please select one and SAVE once!',
                                'Keine Instanz ausgewählt. Bitte auswählen und einmal SPEICHERN!',
                            ),
                        },
                        obj.callback,
                    );
                ...
                const testMsg = this.L(
                    '🔔 *Utility Monitor test*\n\nThis message confirms that your notification settings are correct! 🚀',
                ...
                const sendResult = await new Promise(resolve => {
                    const timeout = this.adapter.setTimeout(() => {
                        resolve({
                            error: this.L(
                                `Timeout: ${instance} did not respond in time. Is the adapter running?`,
                                `Timeout: ${instance} hat nicht rechtzeitig geantwortet. Ist der Adapter aktiv?`,
                            ),
                        });
                    }, 10000);
                    ...
                    this.adapter.sendTo(
                        ...
                        res => {
                            ...
                            if (res && (res.error || res.err)) {
                                resolve({
                                    error: this.L(
                                        `Error from ${instance}: ${res.error || res.err}`,
                                        `Fehler von ${instance}: ${res.error || res.err}`,
                                    ),
                                });
                            } else if (
                                res &&
                                ...
                                resolve({
                                    result: this.L(
                                        `Success! Response from ${instance}: ${JSON.stringify(res)}`,
                                        `Erfolgreich! Antwort von ${instance}: ${JSON.stringify(res)}`,
                                    ),
                                });
                            } else {
                                ...
                                resolve({
                                    result: this.L(
                                        `Test message handed over to ${instance}.`,
                                        `Test-Nachricht an ${instance} übergeben.`,
                                    ),
                                });
                            }
                        },
                ...
                if (obj.callback) {
                    this.adapter.sendTo(
                        obj.from,
                        obj.command,
                        { error: this.L(`Internal error: ${error.message}`, `Interner Fehler: ${error.message}`) },
                        obj.callback,
                    );
                }
            }
    ...
    async checkNotifications() {
        ...
        const typeNames = {
            gas: this.L('Gas', 'Gas'),
            water: this.L('Water', 'Wasser'),
            electricity: this.L('Electricity', 'Strom'),
            pv: 'PV',
        };
            ...
            for (const meter of metersToNotify) {
                ...
                if (this.adapter.config.notificationBillingEnabled) {
                    ...
                    if (billingSent?.val !== true && daysRemaining <= billingDaysThreshold) {
                        const message = this.L(
                            `🔔 *Utility Monitor: read the meter*\n\n` +
                                `Your billing period for *${typeNames[type]} (${meterLabel})* ends in ${daysRemaining} days!\n\n` +
                                `📅 Date: ${periodEnd}\n\n` +
                                `Please enter the meter reading in time:\n` +
                                `1️⃣ Datapoint: ${basePath}.billing.endReading\n` +
                                `2️⃣ Close period: ${basePath}.billing.closePeriod = true`,
                            `🔔 *Nebenkosten-Monitor: Zählerstand ablesen*\n\n` +
                                `Dein Abrechnungszeitraum für *${typeNames[type]} (${meterLabel})* endet in ${daysRemaining} Tagen!\n\n` +
                                `📅 Datum: ${periodEnd}\n\n` +
                ...
                if (this.adapter.config.notificationChangeEnabled) {
                    ...
                    if (changeSent?.val !== true && daysRemaining <= changeDaysThreshold) {
                        const message = this.L(
                            `💡 *Utility Monitor: tariff check*\n\n` +
                                `Your contract for *${typeNames[type]} (${meterLabel})* ends on ${periodEnd}.\n\n` +
                                `⏰ ${daysRemaining} days left until the end of the period.\n\n` +
                                `Now is a good time to compare prices or check the cancellation deadline! 💸`,
                            `💡 *Nebenkosten-Monitor: Tarif-Check*\n\n` +
                                `Dein Vertrag für *${typeNames[type]} (${meterLabel})* endet am ${periodEnd}.\n\n` +
    ...
    async checkMonthlyReport(forceRun = false) {
        ...
        // Generate Report
        const locale = this.lang() === 'de' ? 'de-DE' : 'en-GB';
        let message = this.L(
            `📊 *Monthly report* (${today.toLocaleDateString(locale)})\n\n`,
            `📊 *Monats-Report* (${today.toLocaleDateString(locale)})\n\n`,
        );
        const types = ['electricity', 'gas', 'water', 'pv'];
        const typeNames = {
            electricity: this.L('⚡️ Electricity', '⚡️ Strom'),
            gas: '🔥 Gas',
            water: this.L('💧 Water', '💧 Wasser'),
            pv: '☀️ PV',
        };
        ...
        for (const type of types) {
            ...
            if (isMultiMeter) {
                ...
                message += this.L(`(${meters.length} meters total)\n`, `(${meters.length} Zähler gesamt)\n`);
            ...
            message += this.L(
                `Consumption (year): ${val} ${displayUnit}\n`,
                `Verbrauch (Jahr): ${val} ${displayUnit}\n`,
            );

            // Costs
            const cost = (totalYearlyState?.val || 0).toFixed(2);
            message += this.L(`Consumption costs: ${cost} €\n`, `Verbrauchs-Kosten: ${cost} €\n`);
            ...
            if (paid > 0) {
                ...
                const status =
                    balance > 0
                        ? this.L('❌ Additional payment', '❌ Nachzahlung')
                        : this.L('✅ Credit', '✅ Guthaben');

                message += this.L(`Paid: ${paid.toFixed(2)} €\n`, `Bezahlt: ${paid.toFixed(2)} €\n`);
                message += this.L(`Balance: *${balanceStr} €* (${status})\n`, `Saldo: *${balanceStr} €* (${status})\n`);
            }
